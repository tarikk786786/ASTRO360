...
export default function IslamicAstrologyEngine({ userProfile }: IslamicAstrologyEngineProps) {
      ...
      {activeTab === 'zakat' && (
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
          <div className="glass-card p-8 rounded-3xl border border-white/10 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-white/10">
              <div className="space-y-4">
                <label className="text-xs font-medium text-slate-300 block">Total Eligible Savings & Assets ($ USD):</label>
                <input
                  type="number"
                  value={zakatAssetValue}
                  onChange={(e) => setZakatAssetValue(Number(e.target.value))}
                  className="w-full bg-white/10 border border-white/20 rounded-xl px-4 py-3 text-lg font-bold text-white focus:outline-none focus:border-amber-400"
                />
                <p className="text-[11px] text-slate-400">Nisab Threshold (85g Gold equivalent): ~$5,800 USD</p>
              </div>
              ...
              <div className="p-6 rounded-2xl bg-amber-500/10 border border-amber-500/30 flex flex-col justify-center space-y-2">
                ...
                <p className="text-4xl font-bold font-display text-amber-400">
                  ${(zakatAssetValue * 0.025).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} USD
                </p>
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
          <div className="glass-card p-8 rounded-3xl border border-white/10 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-white/10">
              ...
              <div className="space-y-4">
                <div>
                  <label className="text-xs font-medium text-slate-300 block mb-1">Total Net Estate Value ($):</label>
                  <input
                    type="number"
                    value={estateTotal}
                    onChange={(e) => setEstateTotal(Number(e.target.value))}
                    className="w-full bg-white/10 border border-white/20 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:border-emerald-400"
                  />
                </div>
              ...
              <div className="p-6 rounded-2xl bg-white/5 border border-white/10 space-y-4">
                <h4 className="text-sm font-semibold text-emerald-400 border-b border-white/10 pb-2">Estate Share Breakdown</h4>
                <div className="space-y-2 text-xs">
                  <div className="flex justify-between py-1 border-b border-white/5">
                    <span className="text-slate-300">Wife Share (1/8):</span>
                    <span className="font-bold text-white">${faraidDistribution.wifeShare.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between py-1 border-b border-white/5">
                    <span className="text-slate-300">Each Son Share (2x ratio):</span>
                    <span className="font-bold text-emerald-300">${faraidDistribution.sonShareEach.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between py-1">
                    <span className="text-slate-300">Each Daughter Share (1x ratio):</span>
                    <span className="font-bold text-emerald-300">${faraidDistribution.daughterShareEach.toLocaleString()}</span>
                  </div>
                </div>
              </div>
            </div>
