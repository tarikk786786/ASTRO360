...
export default function NotificationManager({ userProfile, onUpdateProfile }: NotificationManagerProps) {
  ...
  const handleTestDispatch = async () => {
    ...
    const emailSubject = encodeURIComponent(`🚨 AstroVerse Emergency Warning Alert for ${userProfile?.name || 'Seeker'}`);
    const emailBody = encodeURIComponent(
      `ASTROVERSE EMERGENCY COSMIC WARNING & REMEDY DISPATCH\n` +
      `======================================================\n` +
      `Recipient: ${userProfile?.name || 'Seeker'} (${settings.email})\n` +
      `Birth Chart DOB: ${userProfile?.dob || '1998-06-15'}\n` +
      `Alert Type: Immediate Transit Warning / Difficult Planetary Aspect\n\n` +
      `1. TODAY'S TRANSIT WARNING:\n` +
      `   Friction detected in Moon Square Saturn & Rahu aspect.\n\n` +
      `2. ASTROLOGICAL ROOT CAUSE:\n` +
      `   10th & 4th House planetary alignment pressure requiring grounding.\n\n` +
      `3. NEUTRALIZING REMEDY TO DO IMMEDIATELY:\n` +
      `   • Recite "Hasbunallahu wa ni'mal wakeel" (70 times)\n` +
      `   • Give morning Sadaqah or acts of kindness\n` +
      `   • Perform 2 Rakat Salatul Hajah prayer\n\n` +
      `This is an automated emergency warning alert generated by AstroVerse Engine.`
    );
    ...
    try {
      ...
      if (response.ok) {
        setTestStatus(`✅ Emergency Warning Email dispatched to ${settings.email}!`);
      } else {
        // Fallback to mailto link
        window.open(`mailto:${settings.email}?subject=${emailSubject}&body=${emailBody}`, '_blank');
        setTestStatus(`✉️ Opened email client with pre-filled Emergency Warning Alert for ${settings.email}!`);
      }
    } catch {
